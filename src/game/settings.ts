...
export function normalizeSettings(settings: GenerationSettings): GenerationSettings {
  ...
  const rawMineSystem = String((settings as { mineGenerationSystem?: string }).mineGenerationSystem ?? '')
  const mineGenerationSystem = rawMineSystem === 'smart' || rawMineSystem === 'prototypeNoop' ? 'smart' : 'weighted'
  ...
  return { mapSize, mapLayout, hintType, mineGenerationSystem, propagation, snowflakeArms, rectCols, rectRows }
}
